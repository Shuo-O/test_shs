...
MdStore::MdStore(std::string wal_dir, uint32_t trading_day)
    : wal_dir_(std::move(wal_dir)),
      trading_day_(trading_day),
      shm_(new mdv2::Shm()) {}

MdStore::~MdStore() { stop(); }
...
bool MdStore::start() {
    ...
    tailer_.reset(new mdv2::Tailer(&shm_->mapping(), wal_dir_));
    if (!tailer_->start()) {
        error_ = tailer_->error();
        return false;
    }
    started_ = true;
    return true;
...
mdv2::Mapping* MdStore::mapping() { return shm_ ? &shm_->mapping() : nullptr; }

int32_t MdStore::register_instrument(int32_t instrument_id) {
    ...
    if (next_index_ >= kInstrumentCount) return kErrUnknownSymbol;

    int32_t idx = next_index_++;
    ...
    c->sb.instrument_count = static_cast<uint32_t>(next_index_);
    return idx;
}

int32_t MdStore::resolve(int32_t instrument_id) {
...
void MdStore::on_md(const MDUniOrder& order) {
    if (!started_) return;
    ...
    //    cursor; heartbeat shares its line and refreshes on a stride.
