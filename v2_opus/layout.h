...
// Shared-memory layout for the v2 store. Everything here is POD: fixed arrays,
// integer offsets, and lock-free atomics only -- no STL containers, no
// process-local pointers -- so the region is valid across processes mapping it.
//
// Three independently mmap'd regions:
//   ControlRegion : superblock + id index + writer/tailer stat lines  (~3.8 MiB)
//   RingRegion    : per-symbol seqlock rings for latest-n queries     (rings)
//   LogRegion     : append-only day log consumed by the storage tailer
...
// Portable spin hint for seqlock retry loops (PAUSE / YIELD / fallback).
#if defined(__x86_64__) || defined(__i386__)
#  define MDV2_RELAX() __asm__ __volatile__("pause" ::: "memory")
#elif defined(__aarch64__) || defined(__arm__)
#  define MDV2_RELAX() __asm__ __volatile__("yield" ::: "memory")
#else
#  include <thread>
#  define MDV2_RELAX() std::this_thread::yield()
#endif
...
// Line 0: static metadata, written once at init, then read-only. Includes the
...
// Line 2: writer-only monitoring counters, published on a stride (never shares a
// line with the tailer or readers, so the hot path keeps this line in M state).
...
// trusts. They differ: read progress can run ahead of what is on disk.
...
// no separate sequence field needed.
