...
- **撕裂检测**：奇偶 + 见证值不匹配；**覆盖检测**：见证值变大（偶数见证编码了位置，
  覆盖自暴露）。每个被接受的槽都在拷贝前后验证过 `seq == 2*(pos+1)`，因此整组结果
  就是初始 `write_seq` 快照时刻的环内容——**不需要**结尾再复查一次 `write_seq`。
  写者在拷贝期间绕环只会让快照"稍旧"，不会让它不一致；末尾复查反而会在写压下
  误杀一致快照（深查询 n≈capacity 时几乎必然误报），并多拉一次写者热点行。
- 内存序：写者 `release` 与读者 `acquire` 配对；弱内存序（ARM）下两个 `fence` 不可少，
  x86（TSO）上 fence 编译成空操作，零开销。
- n==1（最常见的"最新一条"轮询）走单槽快路径：一次 acquire + 一次 fence，
  跳过分块批量机制。
