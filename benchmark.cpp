...
Stats benchmark_query(const mdsys::Mapping& mapping,
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        auto end = Clock::now();
        if (count <= 0) {
            std::cerr << "query_latest_n failed, count=" << count << "\n";
            std::exit(1);
        }
...
int main(int argc, char** argv) {
    ...
    DemoMd md(options.wal_dir, 20260608);
    if (!md.start()) {
    ...
    for (int i = 0; i < options.symbols; ++i) {
        ...
        md.register_instrument(600000 + i);
    }
    ...
    if (!reader.open(true, false)) {
        std::cerr << "reader open failed: " << reader.error() << "\n";
        return 1;
    }

    Stats query_100 = benchmark_query(reader.mapping(), 600000, 100, options.query_iters);
    ...
    uint64_t committed = md.mapping()->ctrl->wc.committed_seq.load();
    uint64_t durable = md.mapping()->ctrl->tstats.durable_seq.load();
    ...
    std::cout << "durable_seq=" << durable << "\n";
    std::cout << "daylog_overwrite_count="
              << md.mapping()->ctrl->tstats.log_overwrite_count.load() << "\n";
